...
def naityoptimalniyput(n, d, forward_steps, multiplier_steps):
    if n <= 0:
        return 0
    price = [0] * (n + 1)
    for i in range(1, n + 1):
    ...
    for i in range(1, n + 1):
        ...
        for j in range(len(multiplier_steps)):
            if i % multiplier_steps[j] == 0:
                ...
                if current_cost < cost[i]:
                    cost[i] = current_cost
                    predshag[i] = i // multiplier_steps[j]

    optimal_path = []
    current_step = n
    while current_step > 0:
        optimal_path.append(current_step)
        current_step = predshag[current_step]
...
d = int(input("Введите стоимость прыжка: "))
...
price = cenaputi(n, d)
optimal_path, steps = naityoptimalniyput(n, d, forward_steps, multi_steps)
...
plt.scatter(x, y, color='black', marker='o', label='Выбранные ступеньки')
